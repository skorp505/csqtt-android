...
`sudo bash scripts/diagnose_csqtt_server.sh` и
`sudo bash scripts/csqtt-network-report.sh report` собирают состояние служб,
сокетов, интерфейсов и маршрутов. Начиная с 2.1.13 они не включают журнал,
содержимое systemd unit и аргументы процессов: эти источники могут содержать
пароли и токены. Журнал при необходимости изучайте локально отдельно.

В режиме `capture` сетевой скрипт запускает tcpdump; это отдельный захват
сетевого трафика, а не обезличенный отчёт.
